...
def test_hash_u64_nan():
    df_1 = pl.DataFrame({
            "u64": pl.Series([5, 3, None, 20], dtype=pl.UInt64) })
    ...
    df_2 = pl.DataFrame({
        "u64": pl.Series([5, 3, 20, None], dtype=pl.UInt64) })
    ...
    assert not hash_2 == hash_1 #make sure changing the position of the nan changes the result
